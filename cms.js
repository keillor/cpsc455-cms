...
const authVerify = function (req, res, next) {
  ...
  if (path == "/articles/unpublished") {
    if (
      decoded["user"]["role"] == "editor" ||
      decoded["user"]["role"] == "writer"
    ) {
...
app.post("/authenticate", async (req, res) => {
  ...
  if (hash.length == 0) {
    res
      .status(403)
      .send(
        "🚫🔑 INVALID CREDENTIALS 🔑🚫\nOof! Wrong credentials, champ! 😬🔍 Double-check those keys, 'cause this door isn’t budging until you get it right. 🏰💅 Try again… carefully this time! 🤞✨"
      );
    return;
  }
  ...
  if (!userPerms) {
    ...
    res
      .status(404)
      .send(
        "<h1>😔🤷🟪 You have no permissions yet 🟪🤷😔</h1> \n Please contact your administrator"
      );
    return;
  }
